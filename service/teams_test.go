...
func TestService_CompareAllTeamsLists(t *testing.T) {
	...
	type FetcherMockBehavior func(f *Mockfetcher, seasonID int)
	...
	testCases := []struct {
	...
	}{
		{
			...
			FetcherMockBehavior: func(f *Mockfetcher, seasonID int) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, l := range leagues {
				tc.FetcherMockBehavior(fetcherMock, l.CurrentSeasonID)
			}
